...
} = require("../availability/availability.service");
...
const validateBookingInput = ({
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(bookingDate || "")))
    throw new HttpError(400, "Valid booking date is required");
  if (
    !/^\d{2}:\d{2}$/.test(toTime(startTime)) ||
    !/^\d{2}:\d{2}$/.test(toTime(endTime)) ||
...
const assertNoConflict = async (
  ...
  if ((data || []).length > 0)
    throw new HttpError(409, "Booking time is not available");
};
...
const createBooking = async (userId, payload) => {
  ...
  const slotAvailable = slots.some(
    (slot) =>
      toTime(slot.start_time) === startTime &&
      toTime(slot.end_time) === endTime,
  );
  ...
  const { data: booking, error } = await db
    ...
    .select(
      "id, tenant_id, branch_id, field_id, customer_id, booking_date, start_time, end_time, status, total_amount, created_at, updated_at",
    )
    .single();

  // ไฮไลท์อยู่ตรงนี้ครับ! ถ้า Supabase เตะกลับ มันจะฟ้องข้อความให้เราเห็น
  if (error) {
    console.error("🔥 SUPABASE ERROR (BOOKING):", error);
    throw new HttpError(500, "Unable to create booking");
  }
  ...
  const { data: payment, error: paymentError } = await db
    ...
    .select(
      "id, booking_id, tenant_id, amount, status, payment_method, created_at, updated_at",
    )
    .single();

  if (paymentError) {
    console.error("🔥 SUPABASE ERROR (PAYMENT):", paymentError);
    throw new HttpError(500, "Unable to create payment");
  }
...
const getBooking = async (userId, bookingId) => {
  ...
  if (
    ...
    !(await hasTenantRole(db, userId, data.tenant_id))
  ) {
